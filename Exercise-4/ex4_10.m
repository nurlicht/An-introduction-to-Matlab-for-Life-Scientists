function ex4_10()
    ...
    uicontrol(...
        ...
        'Callback', @setFigureVisibility ...
        );
    disp('');
    disp('This command (w/ single quotes) makes a hidden figure visibile:');
    disp('set(gcf,"visible","on");');
    
    %End of program

    %The following function will be used only in response to the pushbutton
    function setFigureVisibility(source, callbackdata)
        h = gcf;
        if (displayFigure == 0)
            displayFigure = 1;
            set(h, 'Visible', 'on');
