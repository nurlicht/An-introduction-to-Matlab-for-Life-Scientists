function ex6_8()
    ...
    % 8. Plotting Surface plot and contours of a 2D section
    ...
    h = figure();
    Define_UIControls(h);
    map2D = getMiddleSection(map3D);
    Plot2SurfaceAndContour(map2D);
    
    function Plot2SurfaceAndContour(map2D)
        subplot(1,2,1);
        meshc(map2D);
        view(-32, 12);
    end

    function map2D = getMiddleSection(map3D)
        mapSize = size(map3D);
        middleZIndex = floor(mapSize(3) / 2);
        map2D = map3D(:,:, middleZIndex);
        map2D = squeeze(map2D);
    end
